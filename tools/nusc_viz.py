...
def lidiar_render(sample_token, data, score_thr, out_path=None):
    bbox_gt_list = []
    bbox_pred_list = []
...
def render_sample_data(
    ...
    """
    lidiar_render(sample_toekn, pred_data, score_thr=score_thr, out_path=out_path)
    ...
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            boxes = [Box(record['translation'], record['size'], Quaternion(record['rotation']),
                     ...
                     pred_data['results'][sample_toekn] if record['detection_score'] > score_thr]
...
if __name__ == '__main__':
    ...
    bevformer_results = mmcv.load(sys.argv[1])
    out_dir = sys.argv[2]
    mmcv.mkdir_or_exist(out_dir)
    sample_token_list = list(bevformer_results['results'].keys())
    for id in range(1, 2):
        render_sample_data(sample_token_list[id], pred_data=bevformer_results,
                           out_path=f'{out_dir}/{id}', score_thr=0.2)