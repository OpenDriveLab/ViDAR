...
class UniADTrack(MVXTwoStageDetector):
    ...
    def _obtain_frozen_history_bev(self, imgs_queue, img_metas_list):
    ...
    def _obtain_backwarded_history_bev(
        ...
        self.eval()
        with torch.no_grad():
            backward_img_feats_list = self.extract_img_feat(
        ...
        for idx, prev_idx in enumerate(range(backwarded_start_idx, backwarded_end_idx)):
            ...
            prev_bev, _ = self.pts_bbox_head.get_bev_features(
                mlvl_feats=img_feats, img_metas=cur_backward_img_metas, prev_bev=prev_bev)
        return prev_bev
    ...
    def get_history_bev(self, imgs_queue, img_metas_list):
        ...
        # Frozen inference.
        if backward_prev_start_idx > 0:
            prev_img = imgs_queue[:, :backward_prev_start_idx, ...]
            prev_img_metas = copy.deepcopy(img_metas_list)
            # prev_bev: bs, bev_h * bev_w, c
            prev_bev = self._obtain_frozen_history_bev(prev_img, prev_img_metas)

        # Backwarded inference.
        if backward_prev_frame_num > 0:
            prev_bev = self._obtain_backwarded_history_bev(
                imgs_queue, prev_bev, copy.deepcopy(img_metas_list),
