...
__global__ void get_grad_sigma_cuda_v2_kernel(
    ...
    const torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> indicator,
    const torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> grad_ray_pred,
    ...
    torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> grad_sigma_regul
    ...
    if (c < M) {
        ...
        for (int i = 0; i < maxlen; i ++) {
            ...
            if (indicator[n][c][i] >= 0){
                atomicAdd(&grad_sigma_regul[n][ts][z][y][x], grad_ray_pred[n][c][i]);
            }
        }
    }
}
 ...
 */
std::vector<torch::Tensor> get_grad_sigma_cuda_v2(
    ...
    auto grad_sigma_regul = torch::zeros_like(sigma_shape);

    AT_DISPATCH_FLOATING_TYPES(sigma_shape.type(), "get_grad_sigma_cuda_v2", ([&] {
                get_grad_sigma_cuda_v2_kernel<scalar_t><<<blocks, threads>>>(
                    ...
                    indicator.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                    grad_ray_pred.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                    grad_sigma.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
                    grad_sigma_regul.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>()
                    );
            }));
    ...
    return {grad_sigma, grad_sigma_regul};
}



template <typename scalar_t>
__global__ void render_cuda_v2_kernel(
    ...
    const torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> tindex,
    const torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> sigma_regul,
    // const torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> occupancy,
    ...
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> ray_pred,
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> indicator
    // torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> grad_sigma_count,
    ) {
    ...
    if (c < M) {
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i ++) {
                ...
                // grad_sigma_count[n][ts][v.z][v.y][v.x] += 1;
                indicator[n][c][i] = 0;
                if (reached_surface != true && d[i] >= gt_d){
                    indicator[n][c][i] = 1;
                    reached_surface = true;
                }
                ray_pred[n][c][i] = sigma_regul[n][ts][v.z][v.y][v.x];
            }
        }
    }
}
 ...
 *   grad_sigma : N x T x H x L x W
 */
std::vector<torch::Tensor> render_cuda_v2(
    torch::Tensor sigma,
    torch::Tensor origin,
    torch::Tensor points,
    torch::Tensor tindex,
    ...
    // -1: ignore; 0: valid; 1: sdf point.
    auto ray_pred = torch::zeros({N, M, MAX_D}, device);
    auto indicator = -torch::ones({N, M, MAX_D}, device);

    AT_DISPATCH_FLOATING_TYPES(sigma.type(), "render_cuda_v2", ([&] {
                render_cuda_v2_kernel<scalar_t><<<blocks, threads>>>(
                    ...
                    tindex.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
                    sigma_regul.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
                    ...
                    indices.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
                    ray_pred.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                    indicator.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>()
                    );
            }));
    ...
    return {pred_dist, gt_dist, dd_dsigma, indices, ray_pred, indicator};
}
